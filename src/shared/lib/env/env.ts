...
> {
	 ...
	 */
	onValidationError?: (
		error: [v.BaseIssue<unknown>, ...v.BaseIssue<unknown>[]],
	) => never;
...
> = Readonly<
	Simplify<
		v.InferOutput<v.ObjectSchema<TServer, undefined>> &
			v.InferOutput<v.ObjectSchema<TClient, undefined>> &
			v.InferOutput<v.ObjectSchema<TShared, undefined>> &
...
>(
	...
	const runtimeEnv =
		opts.runtimeEnvStrict ?? opts.runtimeEnv ?? import.meta.env;
	...
	const parsed = isServer
		? v.safeParse(allServer, runtimeEnv) // on server we can validate all env vars
		: v.safeParse(allClient, runtimeEnv); // on client we can only validate the ones that are exposed

	const onValidationError =
		opts.onValidationError ??
		((error) => {
			const errors = v.flatten(error)?.nested;
			if (!errors) {
	...
	const _isValidServerAccess = (prop: string) => {
